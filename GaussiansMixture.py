...
n_components_range = range(2, 201)
k_range = range(5, 16)
...
for i, n_components in enumerate(n_components_range):
    ...
    for j, k in enumerate(k_range):
        ...
        # Apply Gaussian Mixture Model with diagonal covariance
        ...
        # Visualize the means of the Gaussian models
        if n_components == 2 or n_components == 100 or n_components == 200:
            fig, axs = plt.subplots(nrows=1, ncols=k, figsize=(5*k, 5))
            for l in range(k):
                axs[l].imshow(pca.inverse_transform(gmm.means_[l]).reshape(28, 28), cmap='gray')
                axs[l].set_title('Component {}'.format(l+1))
            plt.suptitle('PCA n_components: {}, k: {}'.format(n_components, k))
            #save the plots
            plt.savefig('GaussiansMixtureMeansPCA{}k{}.png'.format(n_components, k))
...
ax.legend()
#save the plots
plt.savefig('GaussiansMixturePCA.png')
plt.show()
...
np.savez('GaussiansMixture.npz', rand_scores=rand_scores, learning_times=learning_times, n_components_range=n_components_range, k_range=k_range)
